...
import me.montecode.coregame.framework.Input.KeyEvent;
import me.montecode.coregame.framework.Input.TouchEvent;
import me.montecode.coregame.framework.Sound;
...
public class World {
    ...
    private final float SHIELD_FACTOR = 4.0F;
    ...
    private float difficulty = 0.28F; // Max 0.1F
    ...
    private void checkCollisionWithShield(Dot dot, Iterator<Dot> iterator) {
        ...
        if (core.shieldEnergy > 0.0F) {
            ...
            if (dot.type == Dot.Type.Health) {
                core.health += (dot.energy / ENERGY_FACTOR) / 2;
                if (core.health > 1.0F) {
                    core.health = 1.0F;
                }
                coreHealth.play(dot.energy);
            } else if (dot.type == Dot.Type.Shield) {
                core.shieldEnergy += dot.energy / 20;
